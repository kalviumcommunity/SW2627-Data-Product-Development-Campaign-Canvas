...
def test_handle_clerk_callback_email_addresses_format():
    """Test handle_clerk_callback when Clerk returns email in email_addresses dict list format."""
    from src.utils.clerk_auth import handle_clerk_callback
    
    env_mock = {
        "CLERK_CLIENT_ID": "test_id",
        "CLERK_CLIENT_SECRET": "test_secret",
        "CLERK_DOMAIN": "test.clerk.accounts.dev",
        "CLERK_REDIRECT_URI": "http://localhost:8501/"
    }
    
    query_params_mock = MagicMock()
    query_params_mock.__contains__.side_effect = lambda k: k == "code"
    query_params_mock.__getitem__.return_value = "mock_code"
    ...
    userinfo_response.json.return_value = {
        "email_addresses": [{"email_address": "rest_user@clerk.dev"}]
    }
    
    discovery_response = MagicMock(status_code=200)
    discovery_response.json.return_value = {
        "authorization_endpoint": "https://test.clerk.accounts.dev/oauth/authorize",
        "token_endpoint": "https://test.clerk.accounts.dev/oauth/token",
        "userinfo_endpoint": "https://test.clerk.accounts.dev/oauth/userinfo"
    ...
    with patch.dict(os.environ, env_mock), \
        ...
        assert session_state_mock.get("logged_in") is True
        assert session_state_mock.get("email") == "rest_user@clerk.dev"


def test_check_and_restore_session_security_bypass_rejection():
    """Test that check_and_restore_session rejects logged_in=true cookie without valid email."""
    from src.utils.clerk_auth import check_and_restore_session
    
    session_state_mock = MockSessionState()
    # Uninitialized session state
    session_state_mock["logged_in"] = None
    
    context_mock = MagicMock()
    # Cookie has logged_in=true but missing or empty email
    context_mock.cookies = {"logged_in": "true", "email": ""}
    
    with patch.object(st, "session_state", session_state_mock), \
         patch.object(st, "context", context_mock):
        check_and_restore_session()
        assert session_state_mock.get("logged_in") is False
