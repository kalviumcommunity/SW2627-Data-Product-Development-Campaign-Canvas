...
from src.utils.campaigns import fmt_num
from src.utils.clerk_auth import require_authentication
from src.utils.load_css import load_css

st.set_page_config(
    page_title="Data Profiling — CampaignCanvas",
...
def main():
    # Render Sidebar & Top Navbar
    render_sidebar("profiling")
    render_navbar("Data Profiling")

    # Header Banner Card
    st.markdown(
        """
        <div class="glass-card" style="margin-bottom: 1.5rem; padding: 1.25rem; border-radius: 12px;">
            <div style="font-size: 0.75rem; letter-spacing: 0.08em; text-transform: uppercase; color: var(--muted-foreground);">CampaignCanvas</div>
            <div style="font-family: var(--font-display); font-size: 1.5rem; font-weight: 700; color: var(--foreground);">Data Profiling</div>
    ...
    # Quality Score Bar
    st.markdown(
        "<div style='margin-top: 1.5rem; margin-bottom: 0.5rem; font-weight: 600; color: var(--foreground);'>Quality score</div>",
        unsafe_allow_html=True,
    )
    st.progress(quality_score / 100.0)
    st.caption("Completeness ratio minus outlier ratio, scaled from 0 to 100.")

    # Column Profile Data Table
    st.markdown(
        "<div style='margin-top: 2.5rem; margin-bottom: 0.5rem; font-family: var(--font-display); font-size: 1.15rem; font-weight: 700; color: var(--foreground);'>Column profile</div>",
        unsafe_allow_html=True,
    )
